...
func p2pkhOutputs(amounts ...vclutil.Amount) []*wire.TxOut {
...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	currentInputValues := make([]vclutil.Amount, 0, len(unspents))
	f := func(target vclutil.Amount) (vclutil.Amount, []*wire.TxIn, []vclutil.Amount, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			u := unspents[0]
			unspents = unspents[1:]
			nextInput := wire.NewTxIn(&wire.OutPoint{}, nil, nil)
			currentTotal += vclutil.Amount(u.Value)
			currentInputs = append(currentInputs, nextInput)
			currentInputValues = append(currentInputValues, vclutil.Amount(u.Value))
		}
...
func TestNewUnsignedTransaction(t *testing.T) {
	tests := []struct {
		...
		Outputs          []*wire.TxOut
		RelayFee         vclutil.Amount
		ChangeAmount     vclutil.Amount
	...
	for i, test := range tests {
		...
		if tx.ChangeIndex < 0 {
			...
			changeAmount := vclutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
			if test.ChangeAmount == 0 {
