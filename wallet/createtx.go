...
func (s secretSource) GetKey(addr vclutil.Address) (*btcec.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr vclutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb vclutil.Amount, dryRun bool) (
...
func (w *Wallet) addrMgrWithChangeSource(dbtx walletdb.ReadWriteTx,
	...
	changeSource := func() ([]byte, error) {
		...
		var changeAddr vclutil.Address
		var err error
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []vclutil.Amount) error {
