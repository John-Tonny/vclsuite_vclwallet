...
const DefaultRelayFeePerKb vclutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb vclutil.Amount) vclutil.Amount {
	...
	relayFee := vclutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount vclutil.Amount, scriptSize int, relayFeePerKb vclutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb vclutil.Amount) bool {
	...
	return IsDustAmount(vclutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb vclutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > vclutil.MaxSatoshi {
...
func FeeForSerializeSize(relayFeePerKb vclutil.Amount, txSerializeSize int) vclutil.Amount {
	fee := relayFeePerKb * vclutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > vclutil.MaxSatoshi {
		fee = vclutil.MaxSatoshi
	}
