...
func (w *Wallet) MakeMultiSigScript(addrs []vclutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*vclutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*vclutil.AddressScriptHash, error) {
	var p2shAddr *vclutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = vclutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*vclutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
