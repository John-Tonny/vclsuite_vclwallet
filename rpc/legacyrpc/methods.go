...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by vclwallet due to
...
// unsupported by vclwallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &btcjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by vclwallet",
	}
}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*vclutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := vclutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	secp256k1Addrs := make([]vclutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	address, err := vclutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
...
// information about the current state of vclwallet.
// exist.
...
func decodeAddress(s string, params *chaincfg.Params) (vclutil.Address, error) {
	addr, err := vclutil.DecodeAddress(s, params)
	if err != nil {
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := vclutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &btcjson.RPCError{
			Code:    btcjson.ErrRPCInvalidAddressOrKey,
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	var (
		debitTotal  vclutil.Amount
		creditTotal vclutil.Amount // Excludes change
		fee         vclutil.Amount
		feeF64      float64
	)
...
// and amounts.
func makeOutputs(pairs map[string]vclutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := vclutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
func sendPairs(w *wallet.Wallet, amounts map[string]vclutil.Amount,
	account uint32, minconf int32, feeSatPerKb vclutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := vclutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]vclutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]vclutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := vclutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := vclutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	var keys map[string]*vclutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*vclutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := vclutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := vclutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	case *vclutil.AddressPubKeyHash: // ok
		return bytes.Equal(vclutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *vclutil.AddressPubKey: // ok
