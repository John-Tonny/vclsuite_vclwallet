...
func testExternalAddresses(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i := 0; i < len(expectedExternalAddrs); i++ {
			...
			utilAddr, err := vclutil.NewAddressPubKeyHash(
				pkHash, chainParams,
			)
			if err != nil {
...
func testImportPrivateKey(tc *testContext) bool {
	...
	if tc.create {
		for i, test := range tests {
			...
			wif, err := vclutil.DecodeWIF(test.in)
			if err != nil {
...
func testImportScript(tc *testContext) bool {
	...
	testResults := func() bool {
		...
		for i, test := range tests {
			...
			// Use the Address API to retrieve each of the expected
			// new addresses and ensure they're accurate.
			utilAddr, err := vclutil.NewAddressScriptHash(test.in,
				chainParams)
			if err != nil {
...
func testMarkUsed(tc *testContext, doScript bool) bool {
	...
	for i, test := range tests {
		...
		switch test.typ {
		case addrPubKeyHash:
			addr, err = vclutil.NewAddressPubKeyHash(addrHash, chainParams)
		case addrScriptHash:
			addr, err = vclutil.NewAddressScriptHashFromHash(addrHash, chainParams)
		default:
			panic("unreachable")
		}
		if err != nil {
...
func testLookupExpectedAccount(tc *testContext, expectedAccounts map[string]uint32,
	...
	for i, addr := range expectedAddrs {
		addr, err := vclutil.NewAddressPubKeyHash(addr.addressHash,
...
func TestScopedKeyManagerManagement(t *testing.T) {
	...
	_, ok := externalAddr[0].Address().(*vclutil.AddressScriptHash)
	if !ok {
	...
	_, ok = internalAddr[0].Address().(*vclutil.AddressWitnessPubKeyHash)
	if !ok {
		t.Fatalf("wrong type: %T", externalAddr[0].Address())
	}
