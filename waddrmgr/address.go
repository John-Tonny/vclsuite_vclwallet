...
// Address returns the vclutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() vclutil.Address {
	return a.address
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*vclutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return vclutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	...
	if compressed {
		...
		pubKeyHash = vclutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address vclutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		witAddr, err := vclutil.NewAddressWitnessPubKeyHash(
		...
		address, err = vclutil.NewAddressScriptHash(
	...
	case PubKeyHash:
		address, err = vclutil.NewAddressPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
	...
	case WitnessPubKey:
		address, err = vclutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *vclutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() vclutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := vclutil.NewAddressScriptHashFromHash(
