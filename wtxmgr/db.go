...
func fetchMinedBalance(ns walletdb.ReadBucket) (vclutil.Amount, error) {
	...
	return vclutil.Amount(byteOrder.Uint64(v)), nil
}

func putMinedBalance(ns walletdb.ReadWriteBucket, amt vclutil.Amount) error {
	v := make([]byte, 8)
...
func fetchRawCreditAmount(v []byte) (vclutil.Amount, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
...
// credit is spent.
func fetchRawCreditAmountSpent(v []byte) (vclutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return vclutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<0) != 0, nil
}

// fetchRawCreditAmountChange returns the amount of the credit and whether the
// credit is marked as change.
func fetchRawCreditAmountChange(v []byte) (vclutil.Amount, bool, error) {
	if len(v) < 9 {
		str := fmt.Sprintf("%s: short read (expected %d bytes, read %d)",
			bucketCredits, 9, len(v))
		return 0, false, storeError(ErrData, str, nil)
	}
	return vclutil.Amount(byteOrder.Uint64(v)), v[8]&(1<<1) != 0, nil
}
...
// amount is returned.
func spendCredit(ns walletdb.ReadWriteBucket, k []byte, spender *indexedIncidence) (vclutil.Amount, error) {
	...
	return vclutil.Amount(byteOrder.Uint64(v[0:8])), putRawCredit(ns, k, v)
}
...
func unspendRawCredit(ns walletdb.ReadWriteBucket, k []byte) (vclutil.Amount, error) {
	...
	return vclutil.Amount(byteOrder.Uint64(v[0:8])), nil
}
...
func (it *creditIterator) readElem() error {
	...
	it.elem.Amount = vclutil.Amount(byteOrder.Uint64(it.cv))
...
func putDebit(ns walletdb.ReadWriteBucket, txHash *chainhash.Hash, index uint32, amount vclutil.Amount, block *Block, credKey []byte) error {
...
func valueUnminedCredit(amount vclutil.Amount, change bool) []byte {
	v := make([]byte, 9)
	byteOrder.PutUint64(v, uint64(amount))
	if change {
		v[8] = 1 << 1
...
func fetchRawUnminedCreditAmount(v []byte) (vclutil.Amount, error) {
	if len(v) < 9 {
...
func fetchRawUnminedCreditAmountChange(v []byte) (vclutil.Amount, bool, error) {
	if len(v) < 9 {
		str := "short unmined credit value"
		return 0, false, storeError(ErrData, str, nil)
	}
	amt := vclutil.Amount(byteOrder.Uint64(v))
	change := v[8]&(1<<1) != 0
