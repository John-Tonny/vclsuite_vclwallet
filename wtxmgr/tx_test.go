...
var (
	...
	TstRecvTx, _                    = vclutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = vclutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *vclutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := vclutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstDoubleSpendTx := vclutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := vclutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: vclutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: vclutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: vclutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: vclutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
		...
		},
		{
			...
			bal: 0,
			unc: vclutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: vclutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := vclutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance vclutil.Amount,
	...
	checkBalance(vclutil.Amount(initialBalance), false)
	checkBalance(vclutil.Amount(initialBalance), true)
	...
	checkBalance(vclutil.Amount(changeAmount), true)
...
func assertBalance(t *testing.T, s *Store, ns walletdb.ReadWriteBucket,
	confirmed bool, blockHeight int32, exp vclutil.Amount) {
...
func TestOutputLocks(t *testing.T) {
	...
	coinbase := newCoinBase(
		vclutil.SatoshiPerBitcoin, vclutil.SatoshiPerBitcoin*2,
	)
	coinbaseHash := coinbase.TxHash()

	// One of the spends will be unconfirmed.
	const unconfirmedBalance = vclutil.SatoshiPerBitcoin / 2
	...
	const confirmedBalance = vclutil.SatoshiPerBitcoin
