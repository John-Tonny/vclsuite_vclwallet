...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []vclutil.Address) error {
	c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]vclutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []vclutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]vclutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []vclutil.Address, outPoints map[wire.OutPoint]vclutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]vclutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
